...
$access = $con->query('SELECT idaccesscontrol, companyname, companyid FROM accesscontrol_view WHERE accountid = "' . $QPaccountid . '"')->fetch_all(MYSQLI_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["companyid"])) {
        $companyidErr = "You must select a Company";
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" and $companyidErr == NULL) {

    //prepare and bind
    $stmt = $con->prepare("INSERT INTO accesscontrol (accountID, companyID) VALUES (?, ?)");
    $stmt->bind_param("ii", $accountid, $companyid);
    if ($stmt->execute()) {
        echo '<div class=\"row\">
...
<div class="content-title">
    <div class="title">
        ...
        <div class="txt">
            ...
            <p>Give access to an Account's data.</p>
        </div>
    </div>
</div>
...
<script>
    function confirmDelete() {
        return confirm("Are you sure you want to remove access to this account?");
    }
</script>
...
<div class="content-block">
    <div class="table">
        <table>
            <thead>
                <tr>
                    <td>User <?=$accountname?> has access to these Company's records:</td>
                </tr>
            </thead>
            <tbody>
                <?php if (!$access): ?>
                <tr>
                    <td colspan="20" class="no-results">No access.</td>
                </tr>
                <?php endif; ?>


                <?php foreach ($access as $company): ?>
                    <tr>
                    <td><?=$company['companyname']?></td>
                    <td>
                        <!-- Delete form with POST request -->
                         <form method="POST" action="deleteaccesscontrol.php" onsubmit="return confirmDelete();">
                            <input type="hidden" name="accesscontrolid" value="<?=$company['idaccesscontrol']?>">
...
<div class="content-block">
    <form class="form form-medium" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]).'?accountid='.$QPaccountid;?>">
        <label for="companyid" style="color: #9298a0;font-weight: 600;font-size: 14px;">Grant Access To:</label>
            <select id="companyid" name="companyid" style="margin-bottom: 30px;">
            <option value=""> - Select a Company - </option>

            <?php if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "<option value=\"" . $row["idcompany"] . "\">" . $row["companyName"] . " (id:" . $row["idcompany"] . ")</option>";
                }
            }
            ?>
        ...
        <div class="col-sm-2"><span style="color:red;"><?php echo $companyidErr;?></span></div>
        <input type="hidden" id="accountid" name="accountid" value="<?=$QPaccountid?>">
