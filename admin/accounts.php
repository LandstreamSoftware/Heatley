...
$stmt = $con->prepare('SELECT id, companyname, firstname, lastname, username, email, activation_code, role, registered, last_seen, approved FROM accounts_view ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT ?,?');
$types = '';
$params = [];
if ($search) {
    $params[] = &$param3;
...
$stmt->execute();
$stmt->bind_result($result_id, $result_companyname, $result_firstname, $result_lastname, $result_username, $result_email, $result_activation_code, $result_role, $result_registered, $result_last_seen, $result_approved);
// Iterate the results
while($stmt->fetch()) {
    // Add result to accounts array
    $accounts[] = ['id' => $result_id, 'companyname' => $result_companyname, 'firstname' => $result_firstname, 'lastname' => $result_lastname, 'username' => $result_username, 'email' => $result_email, 'activation_code' => $result_activation_code, 'role' => $result_role, 'registered' => $result_registered, 'last_seen' => $result_last_seen, 'approved' => $result_approved];
}
// Delete account
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <td><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=firstname'?>">Name<?=$order_by=='firstname' ? $table_icons[strtolower($order)] : ''?></a></td>
                    <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=email'?>">Company<?=$order_by=='company' ? $table_icons[strtolower($order)] : ''?></td>
...
<!--                <td class="responsive-hidden"><a href="<?=$url . '&order=' . ($order=='ASC'?'DESC':'ASC') . '&order_by=registered'?>">Registered Date<?=$order_by=='registered' ? $table_icons[strtolower($order)] : ''?></a></td> -->
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="responsive-hidden"><?=$account['firstname']." ".$account['lastname']?></td>
                    <td>
                        <?php if ($account['companyname']): ?>
                            <?=htmlspecialchars($account['companyname'], ENT_QUOTES)?>
                        <?php else: ?>
                            <span class="orange">Unassigned</span>
                        <?php endif; ?>
                    </td>
