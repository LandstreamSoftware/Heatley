<?php
//This is a puplic page.

// Include the menupublic.php file
...
<html>
	...
	<body>
		<div class="register">
			...
			<form action="contact-us-process.php" method="post" class="form register-form">
				...
				<div class="form-group">
					<svg class="form-icon-left" width="14" height="14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H418.3c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304H178.3z"/></svg>
					<input class="form-input" type="text" name="emailaddress" placeholder="Email" id="emailaddress" required>
				</div>
				...
				<div class="form-group">
					...
					<input class="form-input" type="text" name="mobilenumber" placeholder="Mobile" id="mobilenumber" required>
				</div>
				...
				<div class="form-group mar-bot-5">
					...
					<textarea class="form-input" style="height:100px;" type="textarea" rows="4" name="message" placeholder="Type your message" id="message" required></textarea>
				</div>
...
<?=template_footer()?>
		...
		<script>
		// AJAX code
		const contactUsForm = document.querySelector('.register-form');
		contactUsForm.onsubmit = event => {
			event.preventDefault();
			fetch(contactUsForm.action, { method: 'POST', body: new FormData(contactUsForm), cache: 'no-store' }).then(response => response.text()).then(result => {
				contactUsForm.querySelector('.msg').classList.remove('error','success');
				if (result.toLowerCase().includes('success:')) {
					contactUsForm.querySelector('.msg').classList.add('success');
					contactUsForm.querySelector('.msg').innerHTML = result.replace('Success: ', '');
				} else if (result.toLowerCase().includes('redirect')) {
					window.location.href = 'index.php';
				} else {
					contactUsForm.querySelector('.msg').classList.add('error');
					contactUsForm.querySelector('.msg').innerHTML = result.replace('Error: ', '');
				}
			});
		};
		</script>	
