...
if (php_sapi_name() !== 'cli' && (!isset($_GET['token']) || $_GET['token'] !== $cron_token)) { 
    ...
    $logMessage = "Create rent invoices: $currentDateTime - \n";
    file_put_contents($logFile, $logMessage, FILE_APPEND);

    // Start output buffering 
    // ob_start(); 

    $result = include 'createrentinvoices.php';

    // if ($result === true) {
    //    $capturedOutput = "createrentinvoices.php ran successfully";
    //} else {
    ...
    $logMessage = "Rent invoice creation complete - \n";
    file_put_contents($logFile, $logMessage, FILE_APPEND);
}