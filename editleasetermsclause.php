...
<?=template_header('Edit Lease Clause')?>
...
$QPleasetermid = $QueryParameters['leasetermid'];

$sql = "SELECT * from leaseterms WHERE idleaseterms = $QPleasetermid and recordOwnerID IN ($accessto)";
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $leasetermname = $row["leaseTermName"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["leasetermgrouping"])) {
        $leasetermgroupingErr = "Clause Grouping is required";
    } else {
        $leasetermgrouping = test_input($_POST["leasetermgrouping"]);
        //check if the field only contains letters dash or white space
        if (!preg_match("/^[\/a-zA-Z-0-9āēīōūĀĒĪŌŪ' ,.]*$/", $leasetermgrouping)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" and $leasetermnameErr == NULL and $leasetermtextErr == NULL and $leasetermgroupingErr == NULL and $recordowneridErr == NULL) {
    ...
    //prepare and bind
    $stmt = $con->prepare("UPDATE leaseterms  SET leaseTermName = ?, leaseTermText = ?, leaseTermGrouping = ?, recordOwnerID = ? WHERE idleaseterms = ?");
    $stmt->bind_param("sssii",$leasetermname, $leasetermtext, $leasetermgrouping, $recordownerid, $QPleasetermid);
    $stmt->execute();


//$sql4 = "UPDATE leaseterms SET leaseTermName = '$leasetermname', leaseTermText = '$leasetermtext', leaseTermGrouping = '$leasetermgrouping' WHERE idleaseterms = $QPleasetermid";
    ...
    if ($stmt->execute() === TRUE) {
        ...
        echo "<div class=\"row\">
           <div class=\"col-sm-2\"><a href=\"listleaseterms.php\" class=\"btn btn-primary\">Back to Lease Terms</a></div>
        </div>
        <div class=\"row\">";
    } else {
    ...
    ?>
    <form class="form form-medium" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"].'?leasetermid='.$QPleasetermid);?>">
    <div class="form-group">
        <label class="form-label col-sm-4" for="leasetermgrouping">Group By: <span class="text-danger">*</span></label>
        <div class="col-sm-6"><input class="form-control" id="leasetermgrouping" type="text" name="leasetermgrouping" value="<?php echo $leasetermgrouping;?>"></div>
        <div class="col-sm-2"><span class="error"><span class="text-danger"><?php echo $leasetermgroupingErr;?></span></div>
    </div>
    <div class="form-group">
        <label class="form-label col-sm-4" for="leasetermname">Name: <span class="text-danger">*</span></label>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-10"><textarea class="form-control" id="leasetermtext" name="leasetermtext" rows="15"><?php echo htmlspecialchars($leasetermtext);?></textarea></div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            ...
            while($row5 = $result5->fetch_assoc()) {
                if($row5["idcompany"] === $recordownerid){
