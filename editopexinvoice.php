...
<?= template_header('Edit OPEX Bill') ?>

<div class="page-title">
    <div class="icon">
        <svg onload="updateItems()" width="20" height="20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><!--!Font Awesome Free 6.5.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
            <path d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z" />
        </svg>
    </div>
    <div class="wrap">
        <h2>Edit OPEX Bill</h2>
...
<div class="block">
    ...
    $QPopexinvoiceid = $QueryParameters['id'];
    ...
    $sql = "SELECT * from transactions_view WHERE idtransaction = $QPopexinvoiceid and recordOwnerID IN ($accessto)";
    ...
    $sql3 = "SELECT idcompany, companyName from companies where companyTypeID IN (2,6) and recordOwnerID IN ($accessto) ORDER BY companyName";
    ...
    $sql5 = "SELECT * from invoicestatus where idinvoicestatus < 5 ORDER BY sortOrder";
    ...
    if ($result->num_rows > 0) {
        ...
        $opexid = $invoicenumber = $transactiondate = $transactioncompanyid = $transactionamount = $transactiontotal = $transactiongst = $invoiceduedate = $transactioncategoryid = $invoicepaiddate = $premisesid = $invoicestatusid = "";
    }

    $opexidErr = $invoicenumberErr = $transactiondateErr = $transactioncompanyidErr = $transactionamountErr = $transactiontotalErr = $transactiongstErr = $invoiceduedateErr = $transactioncategoryidErr = $opexidErr = $invoicepaiddateErr = $premisesidErr = $invoicestatusidErr = "";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["invoicenumber"])) {
            ...
            if (!preg_match("/^[a-zA-Z-0-9,' ]*$/", $invoicenumber)) {
                $invoicenumberErr = "Only letters, dash, comma and spaces allowed";
            }
        }
        ...
        if (empty($_POST["transactionamount"])) {
            $transactionamountErr = "Invoice Cost (excl GST) is required";
        } else {
        ...
        if (empty($_POST["transactiongst"])) {
            $transactiongstlErr = "GST is required";
        } else {
        ...
        //check if the field only contains numbers or NULL - 0 = common to all premises.
        if ($invoicestatusid !== null && !preg_match("/^[0-9-1' ]*$/", $invoicestatusid)) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" and $invoicenumberErr == NULL and $transactiondateErr == NULL and $transactioncompanyidErr == NULL and $transactionamountErr == NULL and $transactiontotalErr == NULL and $transactiongstErr == NULL and $invoiceduedateErr == NULL and $transactioncategoryidErr == NULL and $invoicepaiddateErr == NULL and $premisesidErr == NULL and $invoicestatusidErr == NULL) {
        ...
        if ($invoicepaiddate == '') {
            ...
            transactionGST = '$transactiongst', 
            invoiceDueDate = '$invoiceduedate', 
            transactionCategoryID = $transactioncategoryid,
            invoiceStatusID = 2,
            invoicePaidDate = NULL, 
            premisesID = CASE WHEN $premisesid > -1 THEN '$premisesid' ELSE null END
            WHERE idtransaction = $QPopexinvoiceid";
        } else {
            $sql2 = "UPDATE transactions SET 
            opexID = '$opexid', 
            invoiceNumber = '$invoicenumber', 
        ...
        <tbody>
           ...
           <div class=\"col-sm-2\"><a href=\"listtransactions.php?type=2&opex=" . $opexid . "&searchstring=" . $searchstring . "\" class=\"btn btn-primary\">Back to OPEX Invoices</a></div>
           <div class=\"col-sm-2\"><a href=\"payopexinvoices.php\" class=\"btn btn-primary\">Back to Unpaid OPEX Invoices</a></div>
        </div>
        <div class=\"row\">";
        } else {
        ...
        //$sqlopex = "SELECT buildingID FROM opex WHERE idopex = $opexid";
        //$resultopex = $con->query($sqlopex);
        //while ($rowopex = $resultopex->fetch_assoc()) {
        //    $buildingid = $rowopex["buildingID"];
        //}
        //$sql5 = "SELECT idpremises, unitname, buildingname FROM premises_view WHERE idbuildings = $buildingid ORDER BY unitname";
        //$result5 = $con->query($sql5);

    ?>
        <form class="form form-medium" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . '?id=' . $QPopexinvoiceid); ?>">
            <?php if($invoicestatusid == 4 & ($reconciledamount == $transactiontotal) & $isreconciled == 1) {?>
                <div class="alert alert-success">
                    <span style="font-weight:400; font-size:22px;">Reconciled!</span>
                </div>
            <?php } else {?>
                <div class="alert alert-warning">
                    <?php echo "<span style=\"font-weight:400; font-size:22px;\">$" . ABS($reconciledamount - $transactiontotal) . "</span>" ?> to pay
                </div>
            <?php } ?>
            ...
            <div class="form-group">
                <label class="form-label col-sm-4" for="transactioncompanyid" style="padding-top:5px">Supplier Name: <span class="text-danger">*</span></label>
                <div class="col-sm-6">
            ...
            <div class="form-group">
                <label class="form-label col-sm-4" for="transactionamount" style="padding-top:5px">Amount (excl GST):</label>
            ...
            <div class="form-group">
                <label class="form-label col-sm-4" for="transactiongst" style="padding-top:5px">GST:</label>
                <div class="col-sm-6"><input class="form-control" type="text" name="transactiongst" id="transactiongst" value="<?php echo $transactiongst; ?>" onkeyup="myFunction()" readonly></div>
                <div class="col-sm-2"><span class="error"><span class="text-danger"><?php echo $transactiongstErr; ?></span></div>
            </div>
            <div class="form-group">
                <label class="form-label col-sm-4" for="transactiontotal" style="padding-top:5px">Total Amout Due (incl GST):</label>
...
<!-- Temporaily display the Date Paid and Status fields so they can be edited during testing -->           
            <div class="form-group">
                <label class="form-label col-sm-4" for="invoicepaiddate" style="padding-top:5px">Date Paid:</label>
                <div class="col-sm-6"><input class="form-control" type="date" name="invoicepaiddate" value="<?php echo $invoicepaiddate; ?>"></div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    ...
                    while($row5 = $result5->fetch_assoc()) {
                        if($row5["idinvoicestatus"] == $invoicestatusid){
                            echo "<option value=\"" . $row5["idinvoicestatus"] . "\" selected>". $row5["invoiceStatus"] . "</option>";
                        } else {
                            echo "<option value=\"" . $row5["idinvoicestatus"] . "\">". $row5["invoiceStatus"] . "</option>";
                        }
                    }
                    ?>
                    </select>
            ...
            <?php if(isset($status_code)) { ?>
                <div class="row">
                    <div class="col-sm-10" style="padding-top:40px;"><span style="color:red; font-weight:500;"><?php echo $status_code.": ".$status_text; ?></span></div>
                </div>
            <?php } ?>

            <div class="form-group">
                <div class="col-sm-6"><input class="form-control" type="text" name="searchstring" value="<?php echo $searchstring; ?>" hidden></div>
            </div>
