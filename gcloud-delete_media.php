...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM inspectionmedia WHERE idinspectionmedia = $idfile";
    $result = $con->query($sql);
    
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            $objectname = $row["fileURL"];
    ...
    $stmt = $con->prepare("DELETE FROM inspectionmedia WHERE idinspectionmedia = ?");
    ...
    if ($stmt->execute()) {
        ...
        echo "<div class=\"row\">
            <div class=\"col-sm-2\"><a href=\"".$redirect."\" class=\"btn btn-primary\">Back to inspection record</a></div>
        </div>";
    } else {
