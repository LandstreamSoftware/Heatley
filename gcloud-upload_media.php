...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //echo "Name: " . $_FILES["fileToUpload"]["name"] . "<br>"; //name
    //echo "File Size: " . $file_size . "<br>"; //size
    ...
    if ($header === "\xFF\xD8\xFF\xE0" || $header === "\xFF\xD8\xFF\xE1") {
        ...
        if (!in_array($file_ext, $allowed_types)) {
                die("Invalid file type (" . $file_type . "). Only image files are allowed.");
        }
        ...
        $file_path = "{$premisesid}/{$recordid}/{$new_filename}";
        ...
        if ($uploadOK === 1) {
            ...
            $stmt = $con->prepare("INSERT INTO inspectionmedia (inspectionID, fileURL, mediaType) VALUES (?, ?, ?)");
            $stmt->bind_param("iss", $recordid, $file_path, $mediatype);
