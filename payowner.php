...
$sqluser = "SELECT * FROM accounts_view WHERE id = $accountid";
$resultuser = $con->query($sqluser);

while($rowuser = $resultuser->fetch_assoc()) {
    $recordownerid = $rowuser["companyid"];
	$mycompanyname = $rowuser["companyname"];
}
...
$sql1 = "SELECT invoicenumber FROM transactions_view WHERE invoicenumber LIKE '%PMT-%' and recordOwnerID = $recordownerid ORDER BY invoicenumber DESC LIMIT 1";
$result1 = $con->query($sql1);
if ($result1->num_rows > 0) {
	while($row1 = $result1->fetch_assoc()) {
		$lastinvoicenumber = $row1["invoicenumber"]; //Get the invoice number text
		$invoicenumberint = (int)ltrim($row1["invoicenumber"],"PMT-") + 1; //Get the number and add 1 to it
	}
} else {
	$invoicenumberint = 1;
}
$invoicenumber = "PMT-". str_pad($invoicenumberint,4,"0",STR_PAD_LEFT); //Concatenate the Invoice Number.


$queryString = $_SERVER['QUERY_STRING'];
parse_str($queryString, $parsedQuery);
if (isset($parsedQuery['ownerid'])) {
	$ownerid = $parsedQuery['ownerid'];
}
if (isset($parsedQuery['amount'])) {
	$invoicetotal = $parsedQuery['amount'];
	$transactionamount = $invoicetotal  / 1.15;
	$transactiongst = number_format($invoicetotal - $transactionamount,2);
}
if (isset($parsedQuery['month'])) {
	$month = $parsedQuery['month'];
}
if (isset($parsedQuery['year'])) {
	$year = $parsedQuery['year'];
}
if (isset($parsedQuery['building'])) {
	$building = $parsedQuery['building'];
}

$sql1 = "SELECT * FROM companies where idcompany = $ownerid";
$result1 = $con->query($sql1);
if ($result1->num_rows == 1) {
	while ($row1 = $result1->fetch_assoc()) {
		$companyname = $row1["companyName"];
		$companyid = $row1["idcompany"];
...
$yourreference = "Owner Payment";
$yourcode = $invoicenumber;
$theirparticulars = substr($mycompanyname,0,12);
$theircode = $invoicenumber;
$theirreference = "Owner Payment";
...
<body>
	...
	<div class="content" style="width:50%;">
		...
		<div class="block">
			<form method="post"
				action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . '?ownerid=' . $ownerid . '&month=' . $month . '&year=' . $year . '&building=' . $building); ?>">
				</formclass>
				...
				if ($_SERVER["REQUEST_METHOD"] == "POST" and $bankaccountErr == NULL and $bankaccountnumberErr == NULL and $invoicetotalErr == NULL and $theirparticularsErr == NULL and $theircodeErr == NULL and $theirreferenceErr == NULL and $yourcodeErr == NULL and $yourreferenceErr == NULL) {
					...
					// Get a Premises ID
					$sqlprem = "SELECT idpremises FROM premises WHERE buildingID = $building ORDER BY unitName LIMIT 1";
					$resultprem = $con->query($sqlprem);
					while($rowprem = $resultprem->fetch_assoc()) {
						$premisesid = $rowprem["idpremises"];
					}


					// Create the transaction record for the owner payment
					$transactioncategoryid = 19;
					$invoiceduedate = 
					date_default_timezone_set('Pacific/Auckland');
					$transactiondate = date_format(date_create(),"Y-m-d");
					$stmt = $con->prepare("INSERT INTO transactions (invoiceNumber, transactionDate, transactionCategoryID, transactionCompanyID, transactionAmount, transactionTotal, transactionGST, invoiceDueDate, premisesID, invoiceStatusID, recordOwnerID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        			$stmt->bind_param("ssiidddsiii",  $invoicenumber, $transactiondate, $transactioncategoryid, $companyid, $transactionamount, $invoicetotal, $transactiongst, $transactiondate, $premisesid, $invoicestatusid, $recordownerid);
...
//echo "Response1: " . $response1 . "<br>";
									...
									if ($response1 === false) {
										...
										if (isset($data1['item'])) {
											...
											$stmt->bind_param('issi', $statusid, $paymentid, $currentDate, $transactionid);
											$stmt->execute();
											$stmt->close();
					...
					<div class="row">
           				<div class="col-sm-4"><a href="viewreport.php?ownerid=<?php echo $ownerid?>&month=<?php echo $month?>&year=<?php echo $year?>&building=<?php echo $building?>" class="btn btn-primary">Back to Owner Report</a></div>
        			</div>
					<?php
				} else {
				...
				<div class="row" style="padding-top:20px;">
					...
					<div class="col-md-6 border-top" style="padding-top:20px;">
						<div class="mb-5">
							...
							<div class="mb-3">
								...
								<input type="text" id="companyid" name="companyid" hidden value="<?php echo $companyid;?>">
							<div class="col-sm-12"><span class="error"><span
					...
					<div class="row">
						<div class="col-md-4">
							<div class="mb-3">
								...
								<div class="col-sm-12"><span class="error"><span
									class="text-danger"><?php echo $invoicetotalErr; ?></span>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="col-md-4">
							<div class="mb-3">
								...
								<input type="text" class="form-control custom-border custom-input-height" id="yourreference"
									name="yourreference" value="<?php echo $yourreference; ?>" maxlength="12">
								<div class="col-sm-12"><span class="error"><span
					...
					<div class="row" style="padding-top:60px;">
						<div class="col-sm-6" style="text-align:right;">
							<a href="viewreport.php?ownerid=<?php echo $ownerid?>&month=<?php echo $month?>&year=<?php echo $year?>&building=<?php echo $building?>" class="btn btn-outline-dark">Back to Owner Report</a>
						</div>
						<div class="col-sm-6">
