...
/* invoices
  ...
  echo "Search from: ".$ifModifiedSince." to ".date_format($end, "Y-m-d")."<br>";
  ...
  while ($i < $count) {
    ...
    $xeroLastModified = $transactions[$i]->getUpdatedDateUTC();
    ...
    $bankAccount = $transactions[$i]->getBankAccount("AccountID");
...
//    if ($matchDatePaid >= $searchDate && $matchDatePaid < $SearchDateEnd){ // only show last month's paid bills
        ...
        $lineItems = $transactions[$i]->getLineItems(); // usually an array / ArrayObject

        echo "<tr>
            <td style=\"vertical-align:middle; text-align:center;\">" . $bankAccount . "</a></td>
            <td style=\"vertical-align:middle; text-align:center;\">" . $formattedDate . "</a></td>
    ...
    //    echo "<td style=\"vertical-align:middle; text-align:center;\">" . $formattedDate . "</td>
        echo "<td style=\"vertical-align:middle; text-align:right; padding-right:60px;\">" . number_format($transactions[$i]->getAmountPaid(),2) . "</td>
        </tr>";
        $numTransactions++;
//    }
    $i++;
