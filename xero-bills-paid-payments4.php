...
foreach ($connections as $connection) {
    ...
    foreach ($bankAccounts as $account) {
        ...
        do {
            ...
            $paymentsWhere = 'BankAccount.AccountID == guid("' . $bankAccountId . '")';
 ...
 //           $finalWhere = $where ? ("($where) AND ($paymentsWhere)") : $paymentsWhere;
            ...
            // 4) Render all returned payments for this bank account
            for ($i = 0; $i < $count; $i++) {

                // Xero returns dates like /Date(1700000000000+0000)/
                $xeroDate = $payments[$i]->getDate();
                $formattedDate = '';
        ...
        //        $xeroContact = $payments[$i]->getContact();
        //        $contactName = $xeroContact ? $xeroContact->getName() : '';

                $reference    = $payments[$i]->getReference() ?? '';
        //        $currencyCode = $payments[$i]->getCurrencyCode() ?? '';

        //        $total = $payments[$i]->getTotal();
        //        if ($total === null) {
        //            $subTotal = $payments[$i]->getSubTotal() ?? 0;
        //            $taxTotal = $payments[$i]->getTotalTax() ?? 0;
        ...
        //        echo "<td style=\"vertical-align:middle;\">" . htmlspecialchars($contactName) . "</td>";
