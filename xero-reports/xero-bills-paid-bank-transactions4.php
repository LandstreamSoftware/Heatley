...
foreach ($connections as $connection) {
    ...
    foreach ($bankAccounts as $account) {
        ...
        // 3) Page through transactions for THIS bank account
        $page = 1;

        do {
            // IMPORTANT: constrain to this bank account
            // If you already have $where for date range etc, append this to it.
            $transactionsWhere = 'BankAccount.AccountID == guid("' . $bankAccountId . '") AND Type="SPEND"';

            // Combine your existing $where with the account filter
            $finalWhere = $where ? ("($where) AND ($transactionsWhere)") : $transactionsWhere;
//            $finalWhere = $transactionsWhere;

            $result = $apiInstance->getBankTransactions(
                $xeroTenantId,
                $ifModifiedSince,
                $finalWhere,
                $order,
            ...
            // 4) Render all returned transactions for this bank account
            for ($i = 0; $i < $count; $i++) {
