...
$today = date('d M Y');
$where = implode(' AND ', [
    'FullyPaidOnDate >= DateTime('.$fromDate.')',
    'FullyPaidOnDate <= DateTime('.$toDate.')'
]);
...
foreach ($connections as $connection) {
    ...
    echo "<tr class=\"table-info\">
            <td colspan=\"9\" style=\"font-weight:600; font-size:1.2em; text-align:left;\">"
    ...
    $invoicesWhere = 'Type=="ACCPAY"';

    // Combine your existing $where with the account filter
    $finalWhere = $where ? ("($where) AND ($invoicesWhere)") : $invoicesWhere;
    ...
    do {
        ...
        foreach ($invoices as $invoice) {
                ...
                $xeroDate = $invoice->getFullyPaidOnDate();
                $formattedDate = '';
                ...
                $payeeName = $invoice->getContact()->getName();
                $reference    = $invoice->getReference();
                $lineAmountTypes    = $invoice->getLineAmountTypes();
                $currencyCode = $invoice->getCurrencyCode();
                $subTotal = $invoice->getSubTotal();
                $totalTax = $invoice->getTotalTax();
                $total = $invoice->getTotal();

                $invoiceId   = $invoice->getinvoiceID(); // GUID string
                $status = $invoice->getStatus();
                $Url = $invoice->getUrl();

                echo "<tr>";
                echo "<td style=\"vertical-align:middle; text-align:left;\">" . htmlspecialchars($formattedDate) . "</td>";
                echo "<td style=\"vertical-align:middle;\">" . $invoiceNumber . "</td>"; //Displays as Reference in the Xero UI
                echo "<td style=\"vertical-align:middle;\">" . htmlspecialchars($payeeName) . "</td>";
            //    echo "<td style=\"vertical-align:middle;\">$lineAmountTypes</td>";

                $lineItems = $invoice->getLineItems(); // ArrayObject
            ...
            //    echo "<td style=\"vertical-align:middle; text-align:right; padding-right:25px;\">" . $invoice->getAmountPaid() . "</td>";
