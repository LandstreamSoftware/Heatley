...
foreach ($connections as $connection) {
    ...
    $lineItemsWhere = 'Type=="ACCPAY"';

    // Combine your existing $where with the account filter
    $finalWhere = $where ? ("($where) AND ($lineItemsWhere)") : $lineItemsWhere;
    $finalWhere = null;

    $lineItemsResult = $apiInstance->getLineItems($xeroTenantId, $ifModifiedSince, $finalWhere, $order);
    $lineItems = $lineItemsResult->getLineItems() ?? [];
    ...
    foreach ($lineItems as $lineItem) {
            ...
            $description   = $lineItem->getDescription(); // GUID string
            ...
            echo "<tr>";
            echo "<td style=\"vertical-align:middle; text-align:left;\">" . $description . "</td>";
            echo "<td style=\"vertical-align:middle;\">" . $quantity . "</td>";
            echo "<td style=\"vertical-align:middle;\">" . $unitAmount . "</td>";
            echo "<td style=\"vertical-align:middle;\">" . $accountCode . "</td>";
            echo "<td style=\"vertical-align:middle; text-align:center;\">" . $accountID . "</td>";
            echo "<td style=\"vertical-align:middle; text-align:center;\">" . $item . "</td>";
            echo "</tr>";
    }
}
