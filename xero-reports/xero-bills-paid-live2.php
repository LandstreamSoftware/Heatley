...
try {
    foreach ($connections as $connection) {
        ...
        foreach ($accountsResponse->getAccounts() as $account) {
            ...
            $groupedData[$account->getAccountId()] = [
                'account'   => $account,
                'payments'  => [],
                'invoices'  => []
            ];
...
//echo '</pre>';
        ...
        foreach ($paymentsResponse->getPayments() as $payment) {
            ...
            $groupedData[$accountId]['payments'][] = $payment;

            $invoiceId = $payment->getInvoice()->getInvoiceId();

            if ($invoiceId) {
                $invoiceIds[$invoiceId] = true;
                $invoiceToAccount[$invoiceId] = $accountId;
            }
        }
        ...
        $invoiceIdString = implode(',', array_keys($invoiceIds));
        ...
        $invoicesResult = $accountingApi->getInvoices(
            ...
            null,              // where
            null,              // order
            $invoiceIdString,  // ids  <-- comma-separated InvoiceIDs go HERE
            null,              // invoiceNumbers
            null,              // contactIDs
            null,              // statuses
            1,                 // page (line items included when paging)
            null,              // includeArchived
            null,              // createdByMyApp
            null,              // unitdp
            false              // summaryOnly 
            // (optional pageSize, searchTerm may follow depending on your exact SDK build)
        );

        foreach ($invoicesResult->getInvoices() as $fullInv) {

            // Get InvoiceID
            $invoiceId = $fullInv->getInvoiceId();

            // Store full invoice in lookup table
            $invoiceLookup[$invoiceId] = $fullInv;

            // Find which bank account group this invoice belongs to
            $acct = $invoiceToAccount[$invoiceId] ?? null;

            // Add invoice into the correct groupedData bucket
            if ($acct) {
                $groupedData[$acct]['invoices'][] = $fullInv;
            }
        }
...
echo '</pre>';
    ...
    foreach ($groupedData as $accountId => $group) {

    // Skip bank accounts with no payments
        if (empty($group['payments'])) {
            continue;
        }

        // Account Name
        $accountName = $group['account']->getName();

        // --- Account Header Row ---
        echo '<tr style="background:#f0f0f0;font-weight:bold;">';
        echo '<td colspan="6">' . h($accountName) . '</td>';
        echo '</tr>';

        // --- Payment Rows ---
        foreach ($group['payments'] as $payment) {

            $invRef = $payment->getInvoice();
            $invoiceId = $invRef ? $invRef->getInvoiceId() : null;

            $fullInv = ($invoiceId && isset($invoiceLookup[$invoiceId]))
                ? $invoiceLookup[$invoiceId]
                : null;

            // Date Paid
    //        $datePaid = '';
    //        if ($fullInv && method_exists($fullInv, 'getFullyPaidOnDate') && $fullInv->getFullyPaidOnDate()) {
    //            $datePaid = fmtDate($fullInv->getFullyPaidOnDate());
    //        } else {
    //           $datePaid = fmtDate($payment->getDate());
    //        }

            // Date Paid 2
            $datePaid = '';
            if ($fullInv && method_exists($fullInv, 'getFullyPaidOnDate') && $fullInv->getFullyPaidOnDate()) {
                $datePaid = $fullInv->getFullyPaidOnDate();
                if ($datePaid && preg_match('/\d+/', $datePaid, $matches)) {
                    $seconds = ((int)$matches[0]) / 1000;
                    $datePaid = date('d M Y', (int)$seconds);
                }
            } else {
               $datePaid = fmtDate($payment->getDate());
            }
            ...
            $invoiceNumberOrRef = '';
            if (method_exists($payment, 'getReference') && $payment->getReference()) {
                $invoiceNumberOrRef = $payment->getReference();
            } elseif ($fullInv && $fullInv->getInvoiceNumber()) {
                $invoiceNumberOrRef = $fullInv->getInvoiceNumber();
            }

            // Paid To (Contact Name)
            $paidTo = '';
            if ($fullInv && $fullInv->getContact() && $fullInv->getContact()->getName()) {
                $paidTo = $fullInv->getContact()->getName();
            }

            // Description (Line Items)
            $descParts = [];
            if ($fullInv && $fullInv->getLineItems()) {
                foreach ($fullInv->getLineItems() as $li) {
                    $d = $li->getDescription();
                    if ($d) {
                        $descParts[] = trim($d);
                    }
            ...
            $description = $descParts
                ? implode(' | ', array_values(array_unique($descParts)))
                : '';

            // Currency Code
            $currencyCode = $fullInv && $fullInv->getCurrencyCode()
                ? $fullInv->getCurrencyCode()
                : '';
            ...
            // --- Render Row ---
            echo '<tr>';
            echo '<td>' . h($datePaid) . '</td>';
            echo '<td>' . h($invoiceNumberOrRef) . '</td>';
            echo '<td>' . h($paidTo) . '</td>';
            echo '<td>' . h($description) . '</td>';
            echo '<td>' . h($currencyCode) . '</td>';
            echo '<td style="text-align:right;">' . h($amountFmt) . '</td>';
            echo '</tr>';
        }
