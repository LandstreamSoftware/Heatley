...
foreach ($connections as $connection) {
    ...
    $payments = [];
    do {
        $resp = $apiInstance->getPayments($xeroTenantId, null, $where, $order, $page, $pageSize);
    ...
    foreach ($payments as $p) {
        $inv = $p->getInvoice();
        if ($inv && $inv->getInvoiceID()) {
            $invoiceIds[$inv->getInvoiceID()] = true;
        }
    }
    $invoiceIds = array_keys($invoiceIds);

    // Fetch invoices in chunks, build a lookup map for only ACCPAY invoices
    $invoiceMap = []; // invoiceId => ['number' => ..., 'desc' => ..., 'isAccpay' => bool]
    $chunkSize = 40;

    for ($i = 0; $i < count($invoiceIds); $i += $chunkSize) {
    $chunk = array_slice($invoiceIds, $i, $chunkSize);

    $invResp = $apiInstance->getInvoices(
        $xeroTenantId,
        null,   // ifModifiedSince
        null,   // where
        null,   // order
        $chunk, // iDs
        null, null, null, // invoiceNumbers, contactIDs, statuses
        null,   // page
        false,  // includeArchived
        null,   // createdByMyApp
        2,      // unitdp
        false,  // summaryOnly (need line items for descriptions)
        null,   // pageSize
        null    // searchTerm
    );

    foreach (($invResp->getInvoices() ?? []) as $inv) {
        $invoiceId = $inv->getInvoiceID();
        $isAccpay  = ($inv->getType() === 'ACCPAY');

        // Build a single “item description” string (join non-empty line item descriptions)
        $descs = [];
        $lineItems = $inv->getLineItems();
        if ($lineItems) {
        foreach ($lineItems as $li) {
            $d = $li->getDescription();
            if ($d !== null && trim($d) !== '') $descs[] = trim($d);
        }
        }

        $contactObj = $inv->getContact();

        $invoiceMap[$invoiceId] = [
            'isAccpay' => $isAccpay,
            'number'   => $inv->getInvoiceNumber() ?? '',
            'desc'     => implode(' | ', array_unique($descs)),
            'payee'    => $contactObj ? $contactObj->getName() : '',
        ];
    ...
    foreach ($payments as $p) {
        ...
        if (!$inv || !$inv->getInvoiceID()) continue;

        $invoiceId = $inv->getInvoiceID();
        if (!isset($invoiceMap[$invoiceId]) || !$invoiceMap[$invoiceId]['isAccpay']) {
            continue; // not a bill (ACCPAY) payment
        }

        // Bank account (Payment has an Account object)
        $acct = $p->getAccount();
        $bankAccountName = $acct ? ($acct->getName() ?? '') : '';

        // Payment date
        $paymentDate = $p->getDate(); // often YYYY-MM-DD for payments
        ...
        // Bill number + item description (from invoice lookup)
        $billNumber = $invoiceMap[$invoiceId]['number'];
        $itemDesc   = $invoiceMap[$invoiceId]['desc'];
        $billPayee = $invoiceMap[$invoiceId]['payee'];
        ...
        echo "<tr>";
        // echo "<td>" . htmlspecialchars($bankAccountName) . "</td>";
        echo "<td>" . htmlspecialchars($formattedDate) . "</td>";
        echo "<td>" . htmlspecialchars($billPayee) . "</td>";
        echo "<td>" . htmlspecialchars($billNumber) . "</td>";
        echo "<td>" . htmlspecialchars($itemDesc) . "</td>";
        echo "<td style=\"text-align:center;\">" . htmlspecialchars($currencyCode) . "</td>";
        echo "<td style=\"text-align:right; padding-right:25px;\">" . number_format((float)$amount, 2) . "</td>";
        echo "</tr>";
    }
