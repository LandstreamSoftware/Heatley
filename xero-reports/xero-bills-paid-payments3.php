...
foreach ($connections as $connection) {
    ...
    // 1) Accounts once
    $accountsResponse = $accountingApi->getAccounts($xeroTenantId);
    ...
    foreach ($accountsResponse->getAccounts() as $account) {
        if ($account->getStatus() === 'ACTIVE' && $account->getType() === 'BANK') {
            $bankAccounts[$account->getAccountId()] = [
                'account'        => $account,
                'payments'       => [],
                'seenPaymentIds' => [] // de-dupe
            ];
        }
    }

    // 2) Payments once (driver)
    ...
    foreach ($paymentsResponse->getPayments() as $payment) {
        if (!$payment->getInvoice() || $payment->getInvoice()->getType() !== 'ACCPAY') {
            continue;
        }
        ...
        // De-dupe payments by PaymentID (prevents duplicates from loops/paging/retries)
        $paymentId = $payment->getPaymentId();
        if ($paymentId && isset($bankAccounts[$accountId]['seenPaymentIds'][$paymentId])) {
            continue;
        }
        if ($paymentId) {
            $bankAccounts[$accountId]['seenPaymentIds'][$paymentId] = true;
        }

        $bankAccounts[$accountId]['payments'][] = $payment;

        $invId = $payment->getInvoice()->getInvoiceId();
        if ($invId) {
            $invoiceIds[$invId] = true; // de-dupe invoice IDs for invoice fetch
        }
    }

    // 3) Invoices once (bulk)
    $invoiceMap = [];
    if (!empty($invoiceIds)) {
        foreach (array_chunk(array_keys($invoiceIds), 50) as $chunk) {
            $where2 = implode(' OR ', array_map(
                fn($id) => 'InvoiceID=Guid("' . $id . '")',
                $chunk
            ));

            $invoicesResponse = $accountingApi->getInvoices($xeroTenantId, null, $where2);
    ...
    foreach ($bankAccounts as $bankAccountId => $group) {
        ...
        echo "<tr class=\"table-secondary\">
                ...
                . htmlspecialchars($bankAccount->getName()) .
                "</td>
              </tr>";
        ...
        foreach ($group['payments'] as $payment) {
            ...
            // IMPORTANT: reset formattedDate each row to avoid carryover
            $formattedDate = '';
            $paymentDate = $payment->getDate();
            if ($paymentDate && preg_match('/\d+/', (string)$paymentDate, $matches)) {
                $seconds = ((int)$matches[0]) / 1000;
                $formattedDate = date('d M Y', (int)$seconds);
            } elseif ($paymentDate instanceof \DateTimeInterface) {
            ...
            if ($invoice) {
                ...
                foreach ($invoice->getLineItems() as $li) {
                    $desc = trim((string)$li->getDescription());
                    if ($desc !== '') $descriptions[] = $desc;
                }
                $lineItemsContent = mb_strimwidth(implode(' • ', $descriptions), 0, 500, '…');
            }

            $invoiceNumber = $invoice ? (string)$invoice->getInvoiceNumber() : '';

            echo "<tr>
                    <td style=\"font-weight:300; font-size:.75em; text-align:left;\">{$formattedDate}</td>
                    <td style=\"font-weight:300; font-size:.75em; text-align:left;\">".($invoice ? htmlspecialchars($invoice->getContact()->getName()) : '')."</td>
                    <td style=\"font-weight:300; font-size:.75em; text-align:left;\">".htmlspecialchars($ref)."</td>
                    <td style=\"font-weight:300; font-size:.75em; text-align:left;\">".htmlspecialchars($invoiceNumber)."</td>
                    <td style=\"font-weight:300; font-size:.75em; text-align:left;\">".htmlspecialchars($lineItemsContent)."</td>
                    <td style=\"font-weight:300; font-size:.75em; text-align:center;\">".($invoice ? htmlspecialchars((string)$invoice->getCurrencyCode()) : '')."</td>
                    <td style=\"font-weight:300; font-size:.75em; text-align:right; padding-right:25px;\">".$payment->getAmount()."</td>
                  </tr>";
        }
    }
}
